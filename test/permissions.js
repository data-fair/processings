...
describe('Permissions', () => {
  ...
  it('should create a new processing and work on it as admin of an organization', async () => {
    ...
    assert.rejects(global.ax.user1Koumoul.get('/api/v1/runs', { params: { processing: processing._id } }), (err) => err.status === 403)
    ...
    assert.rejects(global.ax.dmeadusOrg.get('/api/v1/runs', { params: { processing: processing._id } }), (err) => err.status === 403)
    assert.rejects(global.ax.cdurning2.get('/api/v1/runs', { params: { processing: processing._id } }), (err) => err.status === 403)
