<template>
  <v-list dense class="py-0">
    <v-list-item v-for="log in logs" :key="log.date" style="min-height: 26px;">
      <span v-if="log.type === 'error'" class="text-body-2 error--text">
      ...
      <span v-if="log.type === 'warning'" class="text-body-2 warning--text">
        {{ log.msg }}
      </span>
      <span v-if="log.type === 'info'" class="text-body-2">
        {{ log.msg }}
      </span>
      <span v-if="log.type === 'debug'" class="text-caption">
        {{ log.msg }}
      </span>
      <span v-if="log.type === 'task'" class="text-body-2" :class="`${taskColor(log)}--text`">
        {{ log.msg }}
      ...
      <span v-if="log.progressDate" class="text-caption pl-2" style="white-space: nowrap;">- {{ log.progressDate | date('lll') }}</span>
    </v-list-item>
  </v-list>
</template>
