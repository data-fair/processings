...
describe('plugin', () => {
  ...
  it('should install a new plugin then list and remove it', async () => {
    ...
    let res = await superadmin.post('/api/plugins', plugin)
    ...
    await assert.rejects(axAno.get('/api/plugins'), (err: { status: number }) => err.status === 401)
    await assert.rejects(dmeadus.get('/api/plugins'), (err: { status: number }) => err.status === 400)
    await assert.rejects(dmeadus.get('/api/plugins?privateAccess=organization:KWqAGZ4mG'), (err: { status: number }) => err.status === 403)
