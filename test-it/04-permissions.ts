...
describe('processing', () => {
  ...
  it('should create a new processing and work on it as admin of an organization', async function () {
    ...
    await admin1Koumoul.patch(`/api/processings/${processing._id}`, {
      active: true,
      config: {
        datasetMode: 'create',
    ...
    const admin1Processing = (await admin1Koumoul.get(`/api/processings/${processing._id}`)).data
    assert.ok(admin1Processing)
    assert.equal(admin1Processing.userProfile, 'admin')
    const user1Processing = (await contrib1Koumoul.get(`/api/processings/${processing._id}`)).data
    assert.ok(user1Processing)
    assert.equal(user1Processing.userProfile, 'read')
    await assert.rejects(user1Koumoul.get(`/api/processings/${processing._id}`), { status: 403 })
    ...
    await admin1Koumoul.patch(`/api/processings/${processing._id}`, { title: 'test' })
    await assert.rejects(contrib1Koumoul.patch(`/api/processings/${processing._id}`, { title: 'test' }), { status: 403 })
    await assert.rejects(user1Koumoul.patch(`/api/processings/${processing._id}`, { title: 'test' }), { status: 403 })
    ...
    await assert.rejects(dmeadusOrg.get(`/api/processings/${processing._id}`), { status: 403 })
    await assert.rejects(cdurning2.get(`/api/processings/${processing._id}`), { status: 403 })
    ...
    const dmeadusOrgProcessing = (await dmeadusOrg.get(`/api/processings/${processing._id}`)).data
    assert.ok(dmeadusOrgProcessing)
    assert.equal(dmeadusOrgProcessing.userProfile, 'read')
    const cdurning2Processing = (await cdurning2.get(`/api/processings/${processing._id}`)).data
    ...
    await assert.rejects(dmeadus.get(`/api/processings/${processing._id}`), { status: 403 })
    // still no write permissions
    await assert.rejects(dmeadusOrg.patch(`/api/processings/${processing._id}`, { title: 'test' }), { status: 403 })
    await assert.rejects(cdurning2.patch(`/api/processings/${processing._id}`, { title: 'test' }), { status: 403 })
