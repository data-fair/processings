...
describe('plugin-registry', () => {
  ...
  it('should search for plugins (just lastest) on npmjs', async () => {
    const res = await axAno.get('/api/plugins-registry', { params: { q: '@data-fair hello world' } })
  ...
  it('should search for plugins and all their distTag versions on npmjs', async () => {
    const res = await axAno.get('/api/plugins-registry', { params: { q: '@data-fair hello world', showAll: 'true' } })
